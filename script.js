...
function init3DFlightMap() {
        ...
        for (let i = 0; i < data.length - 1; i++) {
            ...
            for (let t = 0; t < segments; t++) {
                ...
                const x = 0.5 * (
                    (2 * p1[0]) +
                    (-p0[0] + p2[0]) * ratio +
                    (2 * p0[0] - 5 * p1[0] + 4 * p2[0] - p3[0]) * ratio * ratio +
                    (-p0[0] + 3 * p1[0] - 3 * p2[0] + p3[0]) * ratio * ratio * ratio
                );
                const y = 0.5 * (
                    (2 * p1[1]) +
                    (-p0[1] + p2[1]) * ratio +
                    (2 * p0[1] - 5 * p1[1] + 4 * p2[1] - p3[1]) * ratio * ratio +
                    (-p0[1] + 3 * p1[1] - 3 * p2[1] + p3[1]) * ratio * ratio * ratio
                );
                const z = 0.5 * (
                    (2 * p1[2]) +
                    (-p0[2] + p2[2]) * ratio +
                    (2 * p0[2] - 5 * p1[2] + 4 * p2[2] - p3[2]) * ratio * ratio +
                    (-p0[2] + 3 * p1[2] - 3 * p2[2] + p3[2]) * ratio * ratio * ratio
                );
                smoothPath.push([x, y, z]);
            }
        }
        smoothPath.push(data[data.length - 1]);
        return smoothPath;
    }

    const SMOOTH_FLIGHT_PATH = getSmoothPath(RAW_DATA, 12); // 12倍の密度でなめらかに
    const isMobile = window.innerWidth <= 768;
    let viewState = isMobile ? { longitude: 135.11, latitude: 33.41, zoom: 7.00, pitch: 59.91, bearing: -7.05 } 
                             : { longitude: 135.14, latitude: 33.40, zoom: 8.18, pitch: 59.86, bearing: -8.40 };
    ...
    deckMapInstance = new deck.DeckGL({
        ...
        layers:[
            new deck.PathLayer({ 
                id: 'flight-path', 
                data:[{ path: SMOOTH_FLIGHT_PATH }], // 生成したなめらかなパスを使用
                getPath: d => d.path, 
            ...
            new deck.ColumnLayer({ 
                id: 'poi-pillars', 
                data: [
                    { position:[134.157893, 33.287018], color:[255, 255, 255, 150] },
                    { position:[136.054945, 33.449765], color:[255, 51, 51, 150] }
                ], 
